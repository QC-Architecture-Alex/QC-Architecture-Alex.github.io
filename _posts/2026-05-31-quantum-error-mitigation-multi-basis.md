...
Full quantum error correction exists, but it is expensive: it demands hundreds of physical qubits per logical qubit and has not yet reached the scale needed for useful chemistry. The near-term alternative is **quantum error mitigation (QEM)**—a set of classical post-processing tricks that squeeze more accurate expectation values out of noisy raw data, at the cost of more circuit executions rather than more qubits.
...
- **Clifford Data Regression (CDR)**: replace the hard non-Clifford gates in your circuit with easy Clifford-gate approximations, run those classically-simulable circuits to get exact reference labels, then fit a linear corrector. Typically 11× the circuit cost.
...
Here is the catch that motivated this project: **every existing benchmark uses a single $Z$-basis observable**, typically $\langle Z_0 \rangle$. Real quantum chemistry is different. A molecular Hamiltonian like the H₂ molecule looks like
...
The core idea of ML-based mitigation is to replace the $2^n$-dimensional probability vector (one entry per bitstring) with a compact summary that still captures the noise structure. For a 4-qubit circuit measured in the $Z$ basis, the feature vector is:
...
This failure crystallized a key lesson: the bottleneck was not the model class (quantum vs. classical) but the **choice of features**. The targeted QRC worked at 4 qubits precisely because its structured probe circuits approximated polynomial features of the probability vector. At 8 qubits, the approximation broke down.

That realization redirected the project toward the question we should have asked first: *what features of the measurement statistics actually carry noise-correction information?* The answer, it turned out, was already in the data we were collecting—from every basis we were measuring.
...
The old approach extracts physics features only from the $Z$-basis measurement. The $\langle ZZ \rangle$ correlator is what the model "sees." But here is the problem: in the transverse-field Ising model $H = -J\sum_i Z_i Z_{i+1} - h\sum_i X_i$, the $\langle ZZ \rangle$ correlator is *anticorrelated* with the $X$-field contribution $\langle H_X \rangle = -h \sum_i \langle X_i \rangle$. The model learns a correction for $\langle ZZ \rangle$ that systematically over-corrects the total energy—introducing sign errors in the $X$ contribution.
...
| **Per-group (ours)** | **1×** | **0.083** | **0.007** |
| ZNE [1,3,5] | 3× | 0.106 | 0.014 |
| Ridge MS all-basis | 3× | **0.038** | **0.004** |
...
The fix follows naturally once you see the problem. When you measure in multiple bases to evaluate $\langle H \rangle$, you already have the measurement statistics from each basis. Extract physics features from *all* of them:
...
This matches ZNE's 3× circuit cost but gives the model explicit information about how each observable changes with noise, enabling implicit Richardson extrapolation within the regression framework.
...
2. **Ridge MS all-basis at 3× cost matches or beats CDR at 11× cost** in all four settings. On H₂ under simple noise: Ridge MS 0.004 vs CDR 0.025—a 6× improvement at less than one-third the cost.

3. **The cross-Hamiltonian transfer is lossless**. We trained a single circuit pool on Ising circuits and applied the learned weights to H₂ labels without retraining—zero degradation. One calibration run covers any Hamiltonian on the same device.

We tested under two noise models: a simple uniform depolarizing model, and a realistic model that includes thermal relaxation ($T_1 = 60\,\mu\text{s}$, $T_2 = 40\,\mu\text{s}$), coherent over-rotation ($\varepsilon = 0.04$ rad), nearest-neighbor $ZZ$ crosstalk, and asymmetric readout errors. The per-group model degrades gracefully under realistic noise; Ridge MS holds up well for small systems.

The scalability story is more nuanced. At 8 qubits, the Ridge MS feature vector is 270-dimensional, but we only have 224 training circuits—underdetermined. Ridge MS exceeds the raw noisy baseline at 8 qubits. The per-group model (90-dimensional at 8 qubits, ratio 2.5×) remains well-determined at all sizes and consistently beats ZNE. **Practical rule**: use Ridge MS only when you have at least 3× more training circuits than feature dimensions.
...
CDR k=5 achieves a remarkably uniform 58–59% improvement at every geometry—the correction works regardless of the molecular configuration, which matters because a potential energy surface is only useful if the errors are consistent across it.
...
This result is where the clean story from H₂ acquires some welcome friction:
...
- **But ridge can also win outright.** At $h/J = 0.5$, where the ground state is $ZZ$-dominated and the noisy→ideal map stays nearly linear, ridge HW-cal (0.069) beats CDR k=5 (0.072)—the first hardware case where a learned model is the single best method.
